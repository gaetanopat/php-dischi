...
var $ = __webpack_require__(/*! jquery */ "./node_modules/jquery/dist/jquery.js"); // Stampare a schermo una decina di dischi musicali (vedi screenshot) in due modi diversi:
...
$(document).ready(function () {
  var url = 'http://localhost:8888/php-dischi/data.php'; // codice da clonare

  var select_artista = $('#template_select').html(); // funzione compilatrice

  var template_select_function = Handlebars.compile(select_artista); // codice da clonare

  var album = $('#template_disco').html(); // funzione compilatrice

  var template_disco_function = Handlebars.compile(album); // per popolare la select
  // chiamata ajax
  ...
  $.ajax({
    ...
    'success': function success(data) {
      ...
      var handlebars_artista; // per la visualizzazione nel template
  ...
  visualizzazioneIniziale(url, template_disco_function); // quando clicco su 'Home'
...
function visualizzazioneIniziale(url, template_disco_function) {
  ...
  $('select option').first().prop('selected', true); // svuoto il container della section

  $('.dischi .container').empty(); // resetto la input checkbox

  $('.ordina').prop('checked', false); // chiamata ajax
  ...
  $.ajax({
    ...
    'success': function success(data) {
      var dischi = JSON.parse(data);
      var handlebars_album; // per la visualizzazione nel template
...
function cercaAlbumArtista(url, dischi, template_disco_function) {
  $('select').change(function () {
    ...
    $('.ordina').prop('checked', false); // prendo il value dell'artista selezionato

    var artista_selezionato = $(this).children("option:selected").val();
    var handlebars_album; // se ho selezionato qualcosa di diverso dalla prima option
...
function ordinaAlbum(url, template_disco_function) {
  // dopo aver cliccato resetto con l'option di default
  $('select option').first().prop('selected', true); // chiamata ajax
  ...
  $.ajax({
    ...
    'success': function success(data) {
      ...
      var dischi = JSON.parse(data); // array vuoto che popolerò dopo

      var sortable = []; // pusho nell'array che sarò così
      // 0: Array(2)
      ...
      var handlebars_album; // prendo la chiave in sortable, es. sortable[key] mi restituisce un array con 2 elementi, il primo con tutti i dati dell'album, il secondo con l'anno
...
__webpack_require__(/*! C:\MAMP\htdocs\php-dischi\src\js\app.js */"./src/js/app.js");
module.exports = __webpack_require__(/*! C:\MAMP\htdocs\php-dischi\src\scss\app.scss */"./src/scss/app.scss");
